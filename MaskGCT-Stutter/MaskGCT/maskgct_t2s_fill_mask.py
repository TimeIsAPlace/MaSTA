...
from llama_nar import DiffLlamaPrefix
...
class MaskGCT_T2S_FillMask(nn.Module):
    def __init__(
        self,
        hidden_size=1024,
        num_layers=16,
        ...
        self.t_mode = (
            cfg.t_mode
            if cfg is not None and hasattr(cfg, "t_mode")
            else "mask_ratio"
        )
        self.fixed_t = (
            cfg.fixed_t
            if cfg is not None and hasattr(cfg, "fixed_t")
            else 1.0
        )
    ...
    def sample_t(self, x0, x_mask, target_mask=None):
        if target_mask is None or self.t_mode == "random":
            t = torch.rand(x0.shape[0], device=x0.device, requires_grad=False)
            return torch.clamp(t, 1e-5, 1.0)
        ...
        if self.t_mode != "mask_ratio":
            raise ValueError(f"Unsupported t_mode: {self.t_mode}")

        valid_mask = x_mask.to(device=x0.device).bool()
        target_mask = target_mask.to(device=x0.device).bool() & valid_mask
        valid_len = valid_mask.sum(dim=1).float().clamp_min(1.0)
        mask_ratio = target_mask.sum(dim=1).float() / valid_len
        mask_ratio = mask_ratio.clamp(1e-5, 1.0)
        return torch.asin(mask_ratio) * (2.0 / np.pi)

    def forward_diffusion(self, x0, t, target_mask=None):
        ...
        if target_mask is not None:
            if target_mask.shape != x0.shape:
                raise ValueError(
                    f"target_mask shape {tuple(target_mask.shape)} must match "
                    f"x0 shape {tuple(x0.shape)}"
                )
            prompt_len = torch.zeros(x0.shape[0], dtype=torch.long, device=x0.device)
            mask = target_mask.to(device=x0.device).bool()
            mask = mask[..., None].to(dtype=xt.dtype)
            xt = (
                xt
    ...
    def loss_t(self, x0, x_mask, t, phone_embedding=None, phone_mask=None, target_mask=None):
        xt, new_t, mask, prompt_len, mask_prob = self.forward_diffusion(x0, t, target_mask=target_mask)
        # xt: (B, T, hidden_size)
        # new_t: (B,)
    ...
    def compute_loss(self, x0, x_mask, phone_embedding=None, phone_mask=None, target_mask=None):
        # x0: (B, T)
        # x_mask: (B, T) mask is 0 for padding
        t = self.sample_t(x0, x_mask, target_mask=target_mask)
        return self.loss_t(
            x0, x_mask, t, phone_embedding, phone_mask, target_mask=target_mask
        )
    ...
    @torch.no_grad()
    def reverse_diffusion_fill_mask(
        self,
        semantic_tokens,
        target_mask,
        phone_id,
        ...
        # semantic_tokens: (B, T), target_mask: (B, T), 1 means fill this token
        if target_mask.shape != semantic_tokens.shape:
            raise ValueError(
                f"target_mask shape {tuple(target_mask.shape)} must match "
                f"semantic_tokens shape {tuple(semantic_tokens.shape)}"
            )

        device = semantic_tokens.device
        bsz, _ = semantic_tokens.shape
        if semantic_mask is None:
            semantic_mask = torch.ones_like(semantic_tokens, dtype=torch.bool, device=device)
        else:
            semantic_mask = semantic_mask.to(device=device).bool()
        if phone_mask is None:
            phone_mask = torch.ones_like(phone_id, dtype=torch.bool, device=device)
        else:
            phone_mask = phone_mask.to(device=device).bool()

        target_mask = target_mask.to(device=device).bool() & semantic_mask
        phone_embedding = self.phone_emb(phone_id)
        mask_token = self.mask_emb(torch.LongTensor([0]).to(device))
        seq = semantic_tokens.clone().masked_fill(target_mask, 0)
        active_mask = target_mask.clone()
        initial_mask_count = active_mask.sum(dim=1)
        start_t = self.sample_t(semantic_tokens, semantic_mask, target_mask=target_mask)
        start_prob = self.mask_prob(start_t).clamp_min(1e-5)

        start_temp = temp
        start_choice_temp = 1.0
        for step in range(n_timesteps):
            progress = step / max(n_timesteps, 1)
            next_progress = (step + 1) / max(n_timesteps, 1)
            t = start_t * (1.0 - progress)
            next_t = start_t * (1.0 - next_progress)

            token = self.cond_emb(seq)
            cur = torch.where(active_mask[..., None], mask_token[:, None, :], token)
            embeds = self.diff_estimator(
                cur,
                t,
                semantic_mask,
            ...
            if cfg > 0:
                uncond_embeds = self.diff_estimator(
                    cur,
                    t,
                    semantic_mask,
                    phone_embedding=phone_embedding[:, phone_embedding.shape[1] :, :],
                    phone_mask=phone_mask[:, phone_mask.shape[1] :],
                )
                pos_emb_std = embeds.std()
                embeds = embeds + cfg * (embeds - uncond_embeds)
                rescale_embeds = embeds * pos_emb_std / embeds.std()
                embeds = rescale_cfg * rescale_embeds + (1 - rescale_cfg) * embeds

            logits = top_k(self.to_logit(embeds), filter_thres)
            annealing_scale = 1.0 - progress
            step_temp = start_temp * annealing_scale
            choice_temp = start_choice_temp * annealing_scale

            if step == n_timesteps - 1:
                sampled_ids = logits.argmax(dim=-1)
            else:
                sampled_ids = gumbel_sample(logits, temperature=max(step_temp, 1e-3))

            seq = torch.where(active_mask, sampled_ids, seq)
            seq = torch.where(target_mask, seq, semantic_tokens)

            probs = logits.softmax(dim=-1)
            scores = probs.gather(2, rearrange(sampled_ids, "b n -> b n 1"))
            scores = rearrange(scores, "b n 1 -> b n")
            scores = choice_temp * gumbel_noise(scores) + scores
            remask_scores = 1 - scores
            remask_scores = remask_scores.masked_fill(
                ~active_mask, -torch.finfo(remask_scores.dtype).max
            )

            next_ratio = self.mask_prob(next_t) / start_prob
            next_mask_count = torch.floor(initial_mask_count.float() * next_ratio).long()
            next_mask_count = torch.minimum(next_mask_count, active_mask.sum(dim=1))
            next_active_mask = torch.zeros_like(active_mask)
            for batch_idx in range(bsz):
                keep_count = int(next_mask_count[batch_idx].item())
                if keep_count <= 0:
                    continue
                indices = remask_scores[batch_idx].topk(keep_count, dim=-1).indices
                next_active_mask[batch_idx, indices] = True

            active_mask = next_active_mask & target_mask
            seq = seq.masked_fill(active_mask, 0)
            seq = torch.where(target_mask, seq, semantic_tokens)
    ...
    def forward(self, x0, x_mask, phone_id=None, phone_mask=None, target_mask=None):
        # x0: (B, T)
        # x_mask: (B, T) mask is 0 for padding
