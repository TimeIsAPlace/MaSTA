...
from g2p.g2p.mandarin import chinese_to_bopomofo, bpmf_to_pinyin
import tqdm
...
def p2i(text, sentence, language):

    return text_tokenizer.phoneme2id(phoneme=text, sentence=sentence, language=language)
...
def get_segment(text: str) -> List[str]:
    ...
    while i < len(types):
        ...
        if flag == 0:
            if types[i] == "event":
                event += text[i] + text[i + 1]
                segments.append((event, "event"))
                event = ""
                i += 1
            else:
                temp_seg += text[i]
