...
from dataset_aishell1 import AISHELL1CachedSemanticTokenDataset, AISHELL1SemanticTokenCollate
from maskgct_utils import build_t2s_fill_mask_model
from utils.util import load_config


CFG_PATH = "./config/maskgct.json"
T2S_MODEL_CKPT = "./ckpt/T2S.safetensors"
CHECKPOINT_PREFIX = "T2S_FillMask"
...
def load_t2s_checkpoint(t2s_model, hps, save_model_path):
    ...
    if keep_training_epochs > 0:
        ckpt_path = Path(save_model_path) / f"{CHECKPOINT_PREFIX}_{keep_training_epochs}_{keep_training_steps}.safetensors"
...
def compute_t2s_fill_mask_loss(t2s_model, batch, criterion):
    logits, final_mask, x0, prompt_len, mask_prob = t2s_model(
        ...
        target_mask=batch["mask_sequence"],
    )
...
@torch.no_grad()
def evaluate_t2s_fill_mask(hps, rank, t2s_model, valid_loader):
...
def train_fill_mask_epoch(
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        with autocast(enabled=fp16_run):
            final_loss, final_mask = compute_t2s_fill_mask_loss(t2s_model, batch, criterion)
        ...
        if global_step % save_interval == 0:
            if valid_loader is not None:
                valid_loss, valid_mask_count = evaluate_t2s_fill_mask(
                ...
                if rank == 0:
                    print(
                        ...
                        f"valid_mask_count:{valid_mask_count:.0f}",
                        flush=True,
                    )
            if rank == 0:
                save_model_path.mkdir(parents=True, exist_ok=True)
                ckpt_path = save_model_path / f"{CHECKPOINT_PREFIX}_{epoch}_{global_step}.safetensors"
...
def train_t2s_fill_mask(rank, n_gpus, hps, train_manifest, valid_manifest):
    ...
    train_dataset = AISHELL1CachedSemanticTokenDataset(train_manifest)
    ...
    train_loader = DataLoader(
        ...
        pin_memory=True,
        collate_fn=AISHELL1SemanticTokenCollate(),
        persistent_workers=num_workers > 0,
    )
    valid_loader = None
    if valid_manifest:
        valid_dataset = AISHELL1CachedSemanticTokenDataset(valid_manifest)
    ...
    for epoch in range(epoch_begin, epochs + 1):
        global_step = train_fill_mask_epoch(
            hps,
            rank,
            epoch,
            t2s_model,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train MaskGCT fill-in-mask T2S with AISHELL-1 cached data.")
...
def main():
    ...
    if not train_manifest and Path("../data/AISHELL-1/semantic_cache/train/manifest.jsonl").exists():
        train_manifest = "../data/AISHELL-1/semantic_cache/train/manifest.jsonl"
    if not valid_manifest and Path("../data/AISHELL-1/semantic_cache/dev/manifest.jsonl").exists():
        valid_manifest = "../data/AISHELL-1/semantic_cache/dev/manifest.jsonl"
